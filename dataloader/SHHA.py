...
class SHHA(data.Dataset):
    def __init__(self, mode, main_transform=None, img_transform=None, gt_transform=None):
        ...
        with open(os.path.join(cfg_data.SHHA_scene_dir,mode,mode+'.txt')) as f:
            lines = f.readlines()
    ...
    def read_image_and_gt(self, fname):
        if self.mode == 'train' or 'val':
            img_path = cfg_data.SHHA_DATA_PATH + '/train/img/' + fname +'.jpg'
            den_path = cfg_data.SHHA_DATA_PATH + '/train/den/' + os.path.splitext(fname)[0] + '.csv'
        if  self.mode == 'test':
            img_path = cfg_data.SHHA_DATA_PATH + '/test/img/' + fname
            den_path = cfg_data.SHHA_DATA_PATH + '/test/den/' + os.path.splitext(fname)[0] + '.csv'
